...
class DSSClient1(object):
    ...
    __all_extractions_endpoint = "Extractions/ReportExtractionGetCompletedByScheduleId(ScheduleId='{}')"
    ...
    def get_all_extractions(self, schedule):
         headers={}
         headers["Prefer"] = "respond-async"
         headers["Authorization"] = "Token "+self.__token__
         url = self.__dss_uri__ + self.__all_extractions_endpoint.format(schedule["ScheduleId"])
         resp, err = self.__get__(url, headers, None)
         if err==None:
             return resp
         else:
            raise Exception("get_all_extractions", err)
